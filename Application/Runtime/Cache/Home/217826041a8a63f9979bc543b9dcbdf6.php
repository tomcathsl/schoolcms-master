...
<div class="am-g my-content" <?php if(!empty($max_width_style)): ?>style="<?php echo ($max_width_style); ?>"<?php endif; ?>>
	<div class="am-u-md-6 am-u-sm-centered">
		<!-- form start -->
		<form class="am-form form-validation m-t-30" method="post" action="<?php echo U('Home/User/Login');?>" request-type="ajax-url" request-value="<?php echo ($referer_url); ?>">
			<fieldset>
				<?php if(in_array('sms', MyC('home_user_reg_state')) or in_array('email', MyC('home_user_reg_state'))): ?><span class=""><?php echo L('user_login_on_accounts_text');?></span>
					<?php if(in_array('sms', MyC('home_user_reg_state'))): ?><a href="<?php echo U('Home/User/RegInfo');?>" class="am-btn am-btn-secondary am-btn-xs am-radius"><?php echo L('user_login_immediately_reg_text');?></a>
					<?php elseif(in_array('email', MyC('home_user_reg_state'))): ?>
						<a href="<?php echo U('Home/User/EmailRegInfo');?>" class="am-btn am-btn-secondary am-btn-xs am-radius"><?php echo L('user_login_immediately_reg_text');?></a><?php endif; ?>
					<hr /><?php endif; ?>
				<label><?php echo L('user_login_accounts_text');?></label>
				<div class="am-form-group">
					<input type="text" name="accounts" class="am-radius" minlength="1" placeholder="<?php echo L('user_login_accounts_text');?>" data-validation-message="<?php echo L('user_login_accounts_format');?>" required />
				</div>
				<label><?php echo L('user_login_pwd_text');?></label>
				<div class="am-form-group">
					<input type="password" name="pwd" class="am-radius" placeholder="<?php echo L('user_login_pwd_text');?>" pattern="<?php echo L('common_regex_pwd');?>" data-validation-message="<?php echo L('user_reg_pwd_format');?>" required />
				</div>
				<p>
					<button type="submit" class="am-btn am-btn-primary am-radius am-btn-sm btn-loading-example" data-am-loading="{loadingText: '<?php echo L('common_login_loading_tips');?>'}"><?php echo L('user_login_submit_text');?></button>
					<a href="<?php echo U('Home/User/ForgetPwdInfo');?>" class="am-fr m-t-5"><?php echo L('user_reg_forget_pwd_text');?></a>
				</p>
			</fieldset>
		</form>
		<!-- form end -->
...
<footer data-am-widget="footer" class="am-footer am-footer-default" data-am-footer="{}" <?php if(!empty($max_width_style)): ?>style="<?php echo ($max_width_style); ?>"<?php endif; ?>>
	...
	<div class="am-footer-miscs">
		<p>由 <a href="http://schoolcms.org/" title="SchoolCMS" target="_blank" class="">SchoolCMS</a> 提供技术支持</p>
		<p>CopyRight©2014-<?php echo date('Y');?> SchoolCMS <?php echo L('common_application_ver');?> Inc.</p>
